...
function pOpenGuide(){
  ...
  var sections=[
    ...
    {icon:'🌞',name:'Buenas Noticias',desc:'Un espacio donde la comunidad comparte noticias positivas, logros personales y momentos que hacen bien para elevar el ánimo colectivo.'},
    ...
    {icon:'🩺',name:'Profesionales',desc:'Directorio de profesionales de salud mental que forman parte de la comunidad Velo. Podés ver sus perfiles, reseñas de la comunidad y contactarlos.'},
  ...
  var cards=sections.map(function(s){
    return '<div style="display:flex;align-items:flex-start;gap:12px;padding:12px 14px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:14px">'
      +'<span style="font-size:22px;flex-shrink:0;line-height:1;margin-top:1px">'+s.icon+'</span>'
      +'<div><div style="font-size:13px;font-weight:700;color:#fff;margin-bottom:3px">'+_escHtml(s.name)+'</div>'
      +'<div style="font-size:12px;color:rgba(255,255,255,.6);line-height:1.6">'+_escHtml(s.desc)+'</div></div>'
      +'</div>';
  }).join('');
  var ov=document.createElement('div');
  ov.id='veloGuideOv';
...
function _startRespiraAmbient(){
  ...
  requestAnimationFrame(function(){ requestAnimationFrame(function(){
    ...
    var canR = cr.width / 2; // actual canvas radius
    var petals = ['🌸','🌼','🌺','✨','🍃','💫','🌿','🌸','✨','🍃','🌼','🌺','🌱','💫'];
    var n = petals.length;
    for(var i = 0; i < n; i++){
      var sz        = 14 + (i * 2.1) % 18;              // emoji size 14–32px
      var dur       = (10 + (i * 2.3) % 10) * 1000;    // 10–20s per expansion cycle
      var startAng  = (i / n) * Math.PI * 2;            // evenly distributed start angles
      ...
      // Wrapper slowly rotates from start angle + small drift (gives circular spread)
      wrap.animate([
